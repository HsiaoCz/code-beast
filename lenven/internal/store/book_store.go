...
import (
	...
	"github.com/HsiaoCz/code-beast/lenven/internal/types"
...
func NewMongoBookingStore(client *mongo.Client) *MongoBookingStore {
	return &MongoBookingStore{
		client: client,
		coll:   client.Database(DBNAME).Collection(DBBOOKCOLL),
	}
}
...
func (b *MongoBookingStore) InsertBooking(ctx context.Context, booking *types.Booking) (*types.Booking, error) {
	...
	booking.ID = resp.InsertedID.(primitive.ObjectID).String()
	return booking, nil
}
...
func (b *MongoBookingStore) UpdateBooking(ctx context.Context, id string, update bson.M) error {
	...
	_, err = b.coll.UpdateByID(ctx, oid, m)
	return err
}
